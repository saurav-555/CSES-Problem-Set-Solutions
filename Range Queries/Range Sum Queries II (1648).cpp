...
int sum_in_seg(int *seg, int l , int h , int tl ,int th, int index){
    ...
    int mid = (tl + th) /2;
    return sum_in_seg(seg, l, h, tl , mid , 2*index + 1) + sum_in_seg(seg, l , h, mid + 1, th , 2*index + 2);
...
int create_seg(const int *a , int *seg , int al, int ah , int index ){
    ...
    int mid = (al + ah) /2;
    seg[index] = create_seg(a, seg, al , mid , 2*index + 1) +create_seg(a, seg, mid + 1, ah , 2*index + 2);
    return seg[index];
}
...
int32_t main() {
    ...
    while(t--){
        ...
        if(o==1){
            ...
            int value = r - a[l];
            a[l] = r;
            update_seg(seg, l, value, 0, n-1, 0);
        }
        else {
            cin>>l>>r;
            l--,r--;
